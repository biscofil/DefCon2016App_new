...
public class MyGradient {
    ...
    private HashMap<Integer, MyGradient.ColorInterval> generateColorIntervals() {
        ...
        if (this.mStartPoints[0] != 0.0F) {
            ...
            colorIntervals.put(Integer.valueOf(0), new MyGradient.ColorInterval(i, this.mColors[0], (float) this.mColorMapSize * this.mStartPoints[0]));
        }

        for (i = 1; i < this.mColors.length; ++i) {
            colorIntervals.put(Integer.valueOf((int) ((float) this.mColorMapSize * this.mStartPoints[i - 1])), new MyGradient.ColorInterval(this.mColors[i - 1], this.mColors[i], (float) this.mColorMapSize * (this.mStartPoints[i] - this.mStartPoints[i - 1])));
        }

        if (this.mStartPoints[this.mStartPoints.length - 1] != 1.0F) {
            i = this.mStartPoints.length - 1;
            colorIntervals.put(Integer.valueOf((int) ((float) this.mColorMapSize * this.mStartPoints[i])), new MyGradient.ColorInterval(this.mColors[i], this.mColors[i], (float) this.mColorMapSize * (1.0F - this.mStartPoints[i])));
        }
    ...
    int[] generateColorMap(double opacity) {
        ...
        MyGradient.ColorInterval interval = (MyGradient.ColorInterval) colorIntervals.get(Integer.valueOf(0));
        int start = 0;

        int i;
        for (i = 0; i < this.mColorMapSize; ++i) {
            if (colorIntervals.containsKey(Integer.valueOf(i))) {
                interval = (MyGradient.ColorInterval) colorIntervals.get(Integer.valueOf(i));
                start = i;
            }
